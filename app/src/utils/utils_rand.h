...
namespace utils 
{
	...
	static i32 GetInt_seeded(i32 min, i32 max, u32 seed)
	{
	...
	static f32 GetFloat(f32 min = 0.0f, f32 max = 1.0f)
	{
	...
	static f32 GetFloat_seeded(f32 min, f32 max, u32 seed)
	{
		std::mt19937 seeded_engine(seed);
		std::uniform_real_distribution<f32> dist(min, max);
		return dist(seeded_engine);
	...
	static f64 GetFloat(f64 min = 0.0, f64 max = 1.0)
	{
	...
	static f64 GetFloat_seeded(f64 min, f64 max, u32 seed)
	{
		std::mt19937 seeded_engine(seed);
		std::uniform_real_distribution<f64> dist(min, max);
		return dist(seeded_engine);
	...
	static f32 GetNormalDist(f32 mean = 0.0f, f32 stddev = 1.0f)
	{
	...
	static f32 GetNormalDist_seeded(f32 mean, f32 stddev, u32 seed)
	{
		std::mt19937 seeded_engine(seed);
		std::normal_distribution<f32> dist(mean, stddev);
		return dist(seeded_engine);
	...
	static bool GetBool(f64 probability = 0.5)
	{
	...
	static bool GetBool(f64 probability, u32 seed)
	{
		std::mt19937 seeded_engine(seed);
		std::bernoulli_distribution dist(probability);
		return dist(seeded_engine);
