...
namespace utils 
{
	...
	f32 Clamp(f32 value, f32 min_val, f32 max_val)
	{
		return Min(Max(value, min_val), max_val);
	}

	f64 Clamp(f64 value, f64 min_val, f64 max_val)
	{
		return Min(Max(value, min_val), max_val);
	}
	...
	f32 InvLerp(f32 a, f32 b, f32 value)
	{
		return (value - a) / (b - a);
	}

	f64 InvLerp(f64 a, f64 b, f64 value)
	{
		return (value - a) / (b - a);
	}
	...
	f64 Map(f64 value, f64 in_min, f64 in_max, f64 out_min, f64 out_max)
	{
		return out_min + (value - in_min) * (out_max - out_min) / (in_max - in_min);
	}

	// Wrap a value to a range [min, max)
	f32 Wrap(f32 value, f32 min_val, f32 max_val)
	{
	...
	// Wrap a value to a range [min, max)
	f64 Wrap(f64 value, f64 min_val, f64 max_val)
	{
	...
	bool Equals(f32 a, f32 b, f32 epsilon = 1e-6f)
	{
		return abs(a - b) < epsilon;
	}

	bool Equals(f64 a, f64 b, f64 epsilon = 1e-9)
	{
		return abs(a - b) < epsilon;
	}
